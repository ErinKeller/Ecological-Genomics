...
# The last term in the model is what is tested.  In this case health. You need to rearrange the order. The last factor will be the "important" factor. What is the impact on health given location. This particular script controls for location by removing variation due to location so you are only left with variation due to health status
...
dim(dds)
#[1] 12954    77  # at > 100; we loose many fewer genes # ~13000 genes in our gene assembly that we mapped to # only about 100 genes never had reads mapped to them, the rest had over 100 reads
...
res <- results(dds, name="locationsub.healthS") # pull out results from these interactions and order by significance
res <- res[order(res$padj),]
head(res)
...
colData$group <- factor(paste0(colData$location, ".", colData$health, ".", colData$score)) # this is where the group variable is defined in this case by column data, by health, and by health score
dds <- DESeqDataSetFromMatrix(countData = countData, colData = colData, design = ~ group) # rather than giving this a specific variable (e.g. health) we give it "group" which is all combination of variables. So for each expression data, it compares variables (e.g. tidal vs. subtidal, healthy v. sick)
dim(dds)
...
res <- results(dds, contrast=list( c("groupint.H.0","groupsub.H.0"), c("groupint.S.1","groupsub.S.1")), listValues=c(1/2, -1/2)) # mean must be zero so we can arbitrarily say one group = +1/2 and the other group -1/2



res <- res[order(res$padj),]
head(res)
# log2 fold change (MAP): 0.5 groupint.H.0+groupsub.H.0 vs 0.5 groupint.S.1+groupsub.S.1 # this is what is "up"
# Wald test p-value: 0.5 groupint.H.0+groupsub.H.0 vs 0.5 groupint.S.1+groupsub.S.1 # this is what is going down
# DataFrame with 6 rows and 6 columns
# baseMean log2FoldChange     lfcSE
# <numeric>      <numeric> <numeric>
...
d <- plotCounts(dds, gene="TRINITY_DN44744_c1_g2_TRINITY_DN44744_c1_g2_i2_g.17686_m.17686", intgroup=(c("health","day","location")), returnData=TRUE) # using d to pull out specific genes and assigns to this d variable (included in d is health, day, location).
d
p <- ggplot(d, aes(x= health, y=count, shape = location)) + theme_minimal() + theme(text = element_text(size=20), panel.grid.major = element_line(colour = "grey"))  # now we can use ggplot and aes assignsy and x axes
